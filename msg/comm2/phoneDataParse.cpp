...
int phoneDataParse::packagePairResult(string &str, string errCode)
{
    cJSON *json;
    ...
    utility.getMAC(macname, mac);

    json = cJSON_CreateObject();

    
    cJSON_AddItemToObject(json, "sysID", cJSON_CreateString("0150"));
    cJSON_AddItemToObject(json, "optCode", cJSON_CreateString("0050"));
    cJSON_AddItemToObject(json, "timeOut", cJSON_CreateString(const_cast<char*>(utility.getCurrentTime().c_str())));
    cJSON_AddItemToObject(json, "errCode", cJSON_CreateString(const_cast<char*>(errCode.c_str())));
    cJSON_AddItemToObject(json, "mac", cJSON_CreateString(const_cast<char*>(mac.c_str())));

    str = cJSON_Print(json);

    string oldstr = "\n";
    utility.trim(str,oldstr, 0);
    oldstr = "\t";
    utility.trim(str, oldstr, 0);
    oldstr = " ";
    utility.trim(str, oldstr, 0);
...
int phoneDataParse::packageWiFiInfo(deque<WiFiInfo> wifilist, string &str)
{
    ...
    cJSON_AddItemToObject(wifiList, "optCode", cJSON_CreateString("0050"));
    cJSON_AddItemToObject(wifiList, "timeOut", cJSON_CreateString(const_cast<char*>(utility.getCurrentTime().c_str())));
